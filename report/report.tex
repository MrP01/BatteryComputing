...
\begin{document}
  ...
  \begin{align}
    \label{eq:fd-scheme-a} \frac{A_{j}^{(n+1)} - A_{j}^{(n)}}{\Delta t} & = D_a \frac{A_{j+1}^{(n+1)} - 2 A_{j}^{(n+1)} + A_{j-1}^{(n+1)}}{(\Delta x)^2}, \quad A_j^{(0)} = 1\,, \\
    \frac{B_{j}^{(n+1)} - B_{j}^{(n)}}{\Delta t}                        & = D_b \frac{B_{j+1}^{(n+1)} - 2 B_{j}^{(n+1)} + B_{j-1}^{(n+1)}}{(\Delta x)^2}, \quad B_j^{(0)} = 0\,,
  \end{align}
  where $A_j^{(n)} \in \R$ denotes the finite difference approximation to the solution $a(x_j, t_n)$ and similarly $B_j^{(n)} \in \R$ the approximation to $b(x_j, t_n)$, where $x_j := j \Delta x = \frac{j}{N_x} L$ are equidistant points in space and $t_n = n \Delta t = \frac{n}{N_t} T$ equidistant points in time.
  ...
  The current may then be obtained through the forward difference approximation
  \begin{equation}
    I(t) = \frac{\partial a}{\partial x}\bigg|_{x=0} \approx \frac{U_1^m - U_0^m}{\Delta x}
  \end{equation}
  ...
  where we recall that $U_k: [-1, 1] \mapsto \R$ denote the Chebyshev polynomials of the second kind, $U_k(\cos \theta) \sin(\theta) = \sin\left((k+1) \theta\right)$, confer \Cref{def:chebpoly2ndkind}.
